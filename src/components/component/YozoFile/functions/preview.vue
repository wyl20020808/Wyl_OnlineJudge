  ...
  >
    <a-col :span="24">
      <a-row
        ...
        "
      >
        <div style="font-weight: bold; font-size: 20px">预览文件功能</div>
      </a-row>
      ...
      <a-row>
        <a-button @click="uploadFile" style="margin-top: 10px">预览文件</a-button>
      </a-row>
    </a-col>
  </a-row>
</template>
...
const uploadFile = async () => {
  if (fileList.value.length > 0) {
    ...
    try {
      const response = await axios.post(`${SERVER_URL}/file/view/local`, formData, {
        headers: {
