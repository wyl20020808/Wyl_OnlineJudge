  ...
  >
    <a-col :span="24">
      <a-row
        ...
        "
      >
        <div style="font-weight: bold; font-size: 20px">PPT编辑功能</div>
      </a-row>
      ...
      <a-row style="margin-top: 10px;margin-bottom: 10px;">
        <a-checkbox v-model:checked="readOnly">只读模式</a-checkbox>
        <a-checkbox v-model:checked="userInfo">加入您的用户信息</a-checkbox>
      </a-row>
      <a-row>
        
        <a-button @click="uploadFile" style="margin-top: 10px">PPT编辑</a-button>
      </a-row>
    </a-col>
  </a-row>
</template>
...
const uploadFile = async () => {
  if (fileList.value.length > 0) {
    ...
    formData.append("readOnly", readOnly.value); // 添加只读模式到表单数据中
    formData.append("userInfo", userInfo.value); // 添加加入用户信息到表单数据中
    formData.append("username", JSON.parse(localStorage.getItem("user")).username);
    formData.append("useravatar", JSON.parse(localStorage.getItem("user")).userpicture);
    console.log(formData.get("userInfo"),'userinfo');
    console.log(formData.get("readOnly"),'readOnly');
    try {
      const response = await axios.post(
        `${SERVER_URL}/file/edit/local`,
        formData,
        {
          headers: {
