  ...
  <div>
    <el-card class="outer-card" v-if="problem">
      <el-card class="inner-card"
        ...
        <div
          class="content"
          v-html="parsedDescription(problem.background)"
        ></div>
      </el-card>
      <el-card class="inner-card"
        ><strong>题目描述</strong>
        <div
          class="content"
          v-html="parsedDescription(problem.description)"
        ></div>
      </el-card>
      <el-card class="inner-card"
        ><strong>输入格式</strong>
        <div
          class="content"
          v-html="parsedDescription(problem.inputformat)"
        ></div>
      </el-card>
      <el-card class="inner-card"
        ><strong>输出格式</strong>
        <div
          class="content"
          v-html="parsedDescription(problem.outputformat)"
        ></div>
      </el-card>
      <div
        class="example-cards"
        v-for="(input, index) in problemSample"
        :key="input.id"
      >
        <el-card class="inner-card">
          <strong>输入样例</strong>
          <el-button
            style="color: white"
            class="copy-button"
            :type="buttonTypes[`copyButtonInput${index}`] || 'primary'"
            @click="
              copyToClipboard('inputExample', index, `copyButtonInput${index}`)
            "
            >复制</el-button
          >
        ...
        <el-card class="inner-card">
          ...
          <el-button
            style="color: white"
            class="copy-button"
            :type="buttonTypes[`copyButtonOutput${index}`] || 'primary'"
            @click="
              copyToClipboard(
                'outputExample',
                index,
                `copyButtonOutput${index}`
              )
            "
            >复制</el-button
          >
      ...
      <el-card class="inner-card"
        ><strong>数据范围</strong>
        <div class="content" v-html="parsedDescription(problem.hint)"></div>
      </el-card>
    </el-card>
  </div>
</template>
